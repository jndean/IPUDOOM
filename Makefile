...
build/doom: $(CPU_OBJ)
	g++ $^ $(CFLAGS) -o build/doom

build/cpu_obj/%.o: src/%.c src/*.h
	gcc src/$*.c $(CFLAGS) -c -o $@

build/cpu_obj/%.o: src/ipu/%.cpp
	gcc src/ipu/$*.cpp $(CFLAGS) -c -o $@

build/ipu_rt.gp: $(IPU_OBJ)
	popc -O2 $^ -o $@

build/ipu_obj/%.gp: src/ipu/%.c
	popc -O2 $^ -o $@

build/ipu_obj/%.gp: src/ipu/%.cpp
	popc -O2 $^ -o $@
